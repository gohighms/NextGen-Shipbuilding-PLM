...
def build_mds_schedule(project_name: str) -> list[dict]:
    ...
    return [
        _task("MDS-001", "Hull Plan", "선체설계부", base_date + timedelta(days=8), 6, [], "MDS", project_name),
        _task("MDS-002", "Midship Section", "선체설계부", base_date + timedelta(days=13), 6, ["MDS-001"], "MDS", project_name),
        _task("MDS-003", "Pipe Routing Review", "배관설계부", base_date + timedelta(days=14), 5, [], "MDS", project_name),
        _task("MDS-004", "Foundation Plan", "철의설계부", base_date + timedelta(days=16), 5, [], "MDS", project_name),
        _task("MDS-005", "Shell Expansion", "선체설계부", base_date + timedelta(days=22), 7, ["MDS-002"], "MDS", project_name),
        _task("MDS-006", "Block Key Plan", "선체설계부", base_date + timedelta(days=28), 6, ["MDS-005"], "MDS", project_name),
        _task("MDS-007", "Equipment Arrangement", "의장설계부", base_date + timedelta(days=24), 7, ["MDS-003", "MDS-004"], "MDS", project_name),
        _task("MDS-008", "Cable Plan", "의장설계부", base_date + timedelta(days=30), 6, ["MDS-007"], "MDS", project_name),
        _task("MDS-009", "Drawing Issue List", "기본설계1부", base_date + timedelta(days=38), 6, ["MDS-006", "MDS-008"], "MDS", project_name),
        _task("MDS-010", "Production Input Package", "생산설계부", base_date + timedelta(days=44), 7, ["MDS-009"], "MDS", project_name),
    ]
